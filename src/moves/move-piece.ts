 ...
 import { isEnPassant } from './special-moves/pawn-special-moves';
  ...
  export function movePiece(cells: Readonly<Cell[]>, startCell: Readonly<Cell>, destinationCell: Readonly<Cell>, lastMove: Readonly<Move | undefined>, possibleMoves: Readonly<Cell[]>, turn: CellColor): { cells: Cell[], success: boolean} {
    ...
    const newCells = cells.map(cell => {
      ...
      if (!isCastlingMove(startCell, destinationCell, turn) && toChessNotation(cell.coordinates) === toChessNotation(destinationCell.coordinates)) {
        ...
        return { ...cell, piece: createPieceFromPrototype(startCell.piece!.color, true, cell.coordinates, startCell.piece!)};
      }
      ...
      if (isCastlingMove(startCell, destinationCell, turn)) {
        ...
        if (isLeft) {
          ...
          if (cell.coordinates.col === 2 && cell.coordinates.row === row) return { ...cell, piece: createPieceFromPrototype(startCell.piece!.color, true, cell.coordinates, startCell.piece!) };
          ...
          if (cell.coordinates.col === 6 && cell.coordinates.row === row) return { ...cell, piece: createPieceFromPrototype(startCell.piece!.color, true, cell.coordinates, startCell.piece!) };
    ...
    if ( (startCell.piece?.symbol === 'wP' && destinationCell.coordinates.row === 0) || (startCell.piece?.symbol === 'bP' && destinationCell.coordinates.row === 7)) {
      const promotedCells = Pawn.pawnPromotion(newCells);
