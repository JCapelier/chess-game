...
export function getStalemateTestBoard(): Cell[] {
  // Empty 8x8 board
  const cells: Cell[] = [];
  ...
  blackKingCell.piece = pieceFactory.createPiece(CellColor.Black, true, PieceType.King);

  // Place White king on f6 (row 6, col 5)
  const whiteKingCell = cells.find(cell => cell.coordinates.row === 6 && cell.coordinates.col === 5)!;
  whiteKingCell.piece = pieceFactory.createPiece(CellColor.White, true, PieceType.King);;

  // Place White queen on g7 (row 7, col 6)
  const whiteQueenCell = cells.find(cell => cell.coordinates.row === 7 && cell.coordinates.col === 6)!;
  whiteQueenCell.piece = pieceFactory.createPiece(CellColor.White, true, PieceType.Queen);;
