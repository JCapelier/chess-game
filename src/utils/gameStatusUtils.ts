...
export function isCheckmate(cells: Cell[], lastMove: Move | undefined, turn: CellColor, gameStatus: GameStatus): boolean {
  ...
  for (const cell of playerCells) {
    const moves = getPossibleMoves(cells, cell, lastMove, turn, gameStatus);
    if (moves.length > 0) {
...
export function isStaleMate(cells: Cell[], lastMove: Move | undefined, turn: CellColor, gameStatus: GameStatus): boolean {
  const { check } = checkForCheck(cells, lastMove, turn, gameStatus);
  if (check) return false;
  ...
  for (const cell of playerCells) {
    if (getPossibleMoves(cells, cell, lastMove, turn, gameStatus).length > 0) {
