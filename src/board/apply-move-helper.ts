...
import { areSameCoordinates } from "../utils/cells-utils";
  ...
  export function applyCastling(board:Board, from: Cell, to: Cell): MoveResult {
    ...
    if (castlingMoves(board).some(destination => areSameCoordinates(destination.coordinates, to.coordinates))) {
  ...
  export function canMove(board: Board, from: Readonly<Cell>, to: Readonly<Cell>, simulation: boolean = false) {
    ...
    return possibleMoves.some(cell => areSameCoordinates(cell.coordinates, to.coordinates));
  }
